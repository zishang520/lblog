    ...
    <div class="row">
        <div class="col-lg-12">
            <ul class="nav nav-tabs nav-tablist" role="tablist">
                <li role="presentation"><a href="{{ $route['index'] }}">文章管理</a></li>
                <li role="presentation" class="active"><a href="{{ $route['draft'] }}">草稿管理</a></li>
            </ul>
            <div class="row">
                    ...
                    <table class="table table-striped table-bordered table-hover dataTable no-footer">
                        ...
                        @if (!$articles->isEmpty())
                            ...
                            <tr>
                                ...
                                <td class="tdcenter">
                                    <a href="{{ url('/article/'.$article->id) }}" target="_blank" title="在新窗口查看">
                                        <img src="{{ url('/l-admin/images/vlog.gif') }}" align="absbottom" border="0" /></a>
                                </td>
                                ...
                                <td class="tdcenter"><a href="{{ url('/admin/comment/artid_index/' . $article->id) }}">{{ $article->hasManyComments->count() }}</a></td>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $('#bysort').change(function() {
                var val = $(this).val();
                if(val != ''){
                    window.open('{{ url('/admin/article/sort_id_index') }}/' + val + '/draft', '_self');
                }
            });
            // 根据标签显示文章
            $('.tag-index').click(function(event) {
                ...
                if(val != ''){
                    window.open('{{ url('/admin/article/tag_index') }}/' + val + '/draft', '_self');
                }
            });
            // 分类
